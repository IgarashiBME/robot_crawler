...
SPACING = 0.4       # distance between lines 
...
# for chestnut collector
SBUS_NEUTRAL = 1024
SBUS_LINEAR_OPT = 1154
SBUS_ANGULAR_RIGHT = 1154
SBUS_ANGULAR_LEFT = 894
SBUS_ANGULAR_K = 130
SBUS_ANGULAR_LIMIT = 130
...
class look_ahead():
    def __init__(self):
        ...
        self.cmdvel = Twist()
        self.array_pub = rospy.Publisher('/sbus_cmd', Int32MultiArray, queue_size = 1)
    ...
    def sbus_publisher(self, bearing, translation, pid):
        if abs(bearing) > YAW_TOLERANCE:
            if bearing >= 0:
                sbus_linear = 1024
                sbus_angular = SBUS_ANGULAR_LEFT
            else:
                sbus_linear = 1024
                sbus_angular = SBUS_ANGULAR_RIGHT
        else:
            sbus_linear = SBUS_LINEAR_OPT *translation
            sbus_angular = SBUS_NEUTRAL - pid *SBUS_ANGULAR_K

            # Angular limit
            if sbus_angular > SBUS_NEUTRAL + SBUS_ANGULAR_LIMIT:
                sbus_angular = SBUS_NEUTRAL + SBUS_ANGULAR_LIMIT
            elif sbus_angular < SBUS_NEUTRAL - SBUS_ANGULAR_LIMIT:
                sbus_angular = SBUS_NEUTRAL - SBUS_ANGULAR_LIMIT

        array = [1024, 1024]
        array[0] = sbus_angular
        array[1] = sbus_linear

        ROS_array = Int32MultiArray(data=array)
        self.array_pub.publish(ROS_array)
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            linear_x, angular_z = self.cmdvel_publisher(steering_ang, translation, pid_value)
            self.sbus_publisher(steering_ang, translation, pid_value)
